...
def subscribe_push(req: PushSubscriptionRequest):
    """
    Store Web Push subscription linked to patient phone number.
    """
    try:
        from database.mongodb import DBHelper
        db = DBHelper()
...
def send_prescription_push(req: SendPushRequest):
    """
    Trigger Web Push Notification for a specific phone number.
    """
    try:
        from database.mongodb import DBHelper
        from agents.push_agent import PushAgent
        ...
        payload = {
            ...
            "body": f"Hello {req.patient_name}, your prescription has been generated by the doctor.",
            "url": "/patient"
        }
        
        success = push_agent.send_push(doc["subscription"], payload)
        return APIResponse(success=success)
    except Exception as e:
