...
## Phase 63: Gemini Function Calling & Clinical Tool-Use Engine `[PRIORITY: CRITICAL — AI ACCURACY]`

### Problem Statement

**Before (with `faster-whisper` + local libraries):**
- `faster-whisper` performed highly-tuned phonetic decoding with injected medical vocabulary (`Dolo 650`, `TDS`, `BD`, `OD`, `Augmentin`, etc.)
- The transcript was clean and medically coherent before being sent to the LLM
- LLM had very little to guess — most entity extraction was already done by deterministic STT

**Now (100% Cloud Gemini STT + Extraction):**
- Gemini transcribes audio → good, but no medical vocabulary injection
- Gemini then extracts a structured JSON from the raw transcript in one **single shot**
- **Single-shot structured output has no reasoning loop** — the model fills gaps by hallucinating medicine names, dosages, or inventing diagnoses not spoken by the doctor
- Output accuracy is significantly lower than the local pipeline

### Solution: Gemini Function Calling (Tool-Use) Pipeline

Instead of a single-shot `generate_content(response_schema=PrescriptionSchema)`, we use **Gemini's Function Calling API** to give the model a set of structured clinical tools it must call to produce each piece of the prescription. This forces the model to:
1. Think step-by-step about what is **explicitly present** in the transcript
2. Call a specific tool per entity (medicines, dosages, diagnoses, patient demographics)
3. Only produce fields it can justify from the transcript — **eliminating hallucination**
4. Return tool results that we validate and assemble into the final PrescriptionSchema JSON

### Architecture: Tool-Use Extraction Flow

```
Transcript
    │
    ▼
Gemini + CLINICAL_TOOLS (multi-turn reasoning loop)
    ├── extract_patient_info()      → { name, age, gender, dob, phone, email }
    ├── extract_chief_complaint()   → { chief_complaint, symptoms[] }
    ├── extract_diagnosis()         → { diagnosis }
    ├── extract_medicine()          → { name, dosage, duration, meal_instruction }
    │   (called once per medicine — iterative)
    ├── extract_lab_tests()         → { tests[] }
    ├── extract_advice()            → { advice[] }
    └── extract_follow_up()         → { follow_up }
    │
    ▼
Tool Executor (Python) — validates & sanitizes each tool output
    │
    ▼
Final assembled PrescriptionSchema JSON → DraftStore
```

### Step 63A.1: Define 7 Clinical Function Tools (`prescription_agent.py`)
- [ ] Create `CLINICAL_TOOLS` as a list of `genai.types.FunctionDeclaration` objects:
  - `extract_patient_info` — name, age, gender, dob, phone, email. Only explicitly spoken.
  - `extract_chief_complaint` — primary complaint + individual symptom list
  - `extract_diagnosis` — single diagnosis explicitly stated. No inference.
  - `extract_medicine` — ONE medicine per call. Model calls this tool once per medicine mentioned.
  - `extract_lab_tests` — list of lab/diagnostic tests ordered
  - `extract_advice` — general health advice and dietary instructions
  - `extract_follow_up` — follow-up timeframe or instructions

### Step 63A.2: Multi-Turn Tool-Use Extraction Loop (`generate_prescription_with_tools`)
- [ ] Add `generate_prescription_with_tools(transcript, model_override)` to `PrescriptionAgent`:
  - Prepend `MEDICAL_CONTEXT_HEADER` vocabulary to transcript
  - Send to Gemini with `tools=[CLINICAL_TOOLS]`, `tool_config="AUTO"`
  - Loop while response has `function_calls`: execute tool → feed result back → continue
  - Max 12 iterations safety guard, then fall back to single-shot schema extraction
  - Assemble all tool outputs into PrescriptionSchema-compatible dict

### Step 63A.3: Tool Call Executor & Validator (`_execute_tool_call`)
- [ ] Python-side validator per tool call:
  - Sanitize empty strings to `None`
  - Validate phone number format (10+ digits)
  - Validate email format with regex
  - Reject medicine names that are generic placeholders (`"medicine"`, `"drug"`, `"tablet"`)
  - Return clean validated dict

### Step 63A.4: Medical Vocabulary Context Header (`MEDICAL_CONTEXT_HEADER`)
- [ ] Inject clinical vocabulary as transcript prepend (replaces `faster-whisper` vocabulary injection):
  ```
  [MEDICAL CONTEXT — Indian Clinical Terminology]
  Dosage Codes: BD=Twice Daily, TDS=Three Times Daily, OD=Once Daily, QID=Four Times Daily, HS=At Bedtime, SOS=As Needed
  Common Drugs: Dolo 650, Crocin, Combiflam, Pan 40, Omeprazole, Azithromycin 500mg, Augmentin 625,
    Cetirizine 10mg, Montek LC, Allegra 120, Amoxicillin 500mg, Paracetamol, Ibuprofen, Pantoprazole
  Meal Instructions: After Meals, Before Food, Empty Stomach, With Milk
  [END MEDICAL CONTEXT]
  ```

### Step 63C: Fallback Safety Chain
- [ ] Ensure graceful fallback chain:
  `gemini-2.5-flash (tool-use)` → `gemini-2.5-flash (single-shot schema)` → `gemini-2.5-flash-lite` → `heuristic-regex`

### Step 63D: SpeechAgent Medical Vocabulary Injection (`speech_agent.py`)
- [ ] Upgrade `SpeechAgent.speech_to_text()` Gemini audio call with `STT_MEDICAL_SYSTEM_INSTRUCTION`:
  - Inject medical drug name vocabulary and dosage shorthand into audio transcription
  - Replaces the `faster-whisper` initial prompt vocabulary injection removed in Phase 62
  - Output clean English text even when audio is Hinglish/mixed

### Step 63E: End-to-End Testing
- [ ] Test with sample Hindi/English clinical transcript — verify medicines, dosage, demographics all populate correctly
- [ ] Compare single-shot vs tool-use output on same transcript

### Step 63F: Documentation & Git Sync
- [ ] Commit and push Phase 63 changes
- [ ] Update `index.md`, `progress.md`, `brain.md`, `MASTER_PROMPT.md`

### Files Modified in Phase 63
| File | Change |
|:-----|:-------|
| `backend/agents/prescription_agent.py` | Add `CLINICAL_TOOLS`, `MEDICAL_CONTEXT_HEADER`, `generate_prescription_with_tools()`, `_execute_tool_call()` |
| `backend/agents/speech_agent.py` | Inject `STT_MEDICAL_SYSTEM_INSTRUCTION` into `speech_to_text()` |

### Expected Accuracy Improvement
| Metric | Before (single-shot) | After (tool-use) |
|:-------|:--------------------|:-----------------|
| Medicine hallucination | ~30% of calls | Near zero — each medicine is a forced tool call |
| Dosage accuracy | Moderate | High — model cannot invent dosages it didn't hear |
| Patient demographics | Inconsistent | Consistent — dedicated tool per demographic field |
